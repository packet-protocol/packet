...
const parseSendContentType = (value: unknown): SendContentType => {
  try {
    ...
    throw new Error("contentType must be auto, text, url, irys, ipfs, or arweave");
  }
};
...
export const resolveSendContent = (
  ...
  try {
    ...
    if (err instanceof Error && err.message === "content type url requires an http(s) URL") {
      throw new Error("contentType url requires an http(s) URL in content");
    }
    throw err;
  }
};
...
const assertInlineSize = (byteLength?: number, ignoreWarnings?: boolean): void => {
  ...
  throw new Error("Content too large for inline message body. Upload it to Irys or pass ignoreWarnings to proceed.");
};
...
const prepareUploadedContent = async (
  ...
  if (explicit !== "auto" && explicit !== "irys" && explicit !== "text") {
    throw new Error(`upload is only supported for Irys (contentType irys or text). Got contentType=${explicit}.`);
  }

  if (params.source === "url" && params.ignoreWarnings !== true) {
    throw new Error("Cannot upload content from URL source. Fetch it and provide it as text/file, or pass ignoreWarnings.");
  }
